    ...
    jQuery(function($) {
        ...
        Ytable.prototype.initFilter = function() {
            ...
            $filterButton.on('click', function(ev) {
                // 判断当前是否有条件调用查询
                if(self.canFilter&&(self.canFilter()===false)){
                    return;
                }
                // 阻止默认事件
                ev.preventDefault();
         ...
         */
        // 表格数据的处理，表头和表格  这一步只是生成了数据，并没有对数据进行操作以及将数据写入DOM结构中
        Ytable.prototype.dealData = function(rawData) {
            var self = this;
            var thData = [];
            var trData = [];

            // 生成表头数据，通过遍历的方法  先获取表头的数据，放在一个数组里边
